...
public class QuestionController {
    ...
    public HttpResp list(@RequestParam int pageNo, @RequestParam int pageSize, @RequestParam String search) {
        HttpResp resp = new HttpResp();
        QuestionPo po = new QuestionPo();

        List<String> orderBysList = new ArrayList<String>();
        try {
            resp.setData(service.getDataByPage(pageNo, pageSize, po, orderBysList, search));
        } catch (Exception e) {
            resp.setCode(IoTErrorCode.server_error.getErrorCode());
            resp.setMessage("习题信息列表 查询失败，数据库出错 e= " + e.getMessage());
            return resp;
    ...
    public HttpResp deleteQuestionPo(@PathVariable(value = "id") Long id,
